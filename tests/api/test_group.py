...
def test_private_group_access(authenticated_client, private_group):
    response = authenticated_client.get(f"/api/groups/{private_group.id}/?passcode=test")
    assert response.status_code == 200
...
def test_update_group(authenticated_client, private_group):
    response = authenticated_client.patch(f"/api/groups/{private_group.id}/", {"name": "Updated Group"})
    assert response.status_code == 200
    assert response.data["name"] == "Updated Group"


@pytest.mark.django_db
def test_non_member_cannot_access_private_group(authenticated_client, group_without_users):
    response = authenticated_client.get(f"/api/groups/{group_without_users.id}/?passcode=test")
    assert response.status_code == 403


@pytest.mark.django_db
def test_non_admin_cannot_delete_group(authenticated_client, group_with_member_user):
    response = authenticated_client.delete(f"/api/groups/{group_with_member_user.id}/?passcode=test")
    assert response.status_code == 403
