...
user_data: dict = dict(
    username="kostas2370", email="kostas2372@gmail.com", password="Pass1234!"
)
...
def private_group(user):
    group = Group.objects.create(
        name="Test Group", is_private=True, passcode="test", tags=[]
    )
...
def public_group(user):
    group = Group.objects.create(
        name="Test Group2", is_private=False, passcode=None, tags=[]
    )
    GroupUser.objects.create(role="admin", user=user, group=group)
    return group
